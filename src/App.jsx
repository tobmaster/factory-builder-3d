  ...
  return (
    <>
      ...
      {running && Array.from({ length: itemCount }).map((_, i) => {
        const p = (time * speed + i / itemCount) % 1;
        return <mesh key={i} position={[from.x + dx * p, 0.38, from.z + dz * p]}><boxGeometry args={[0.28, 0.22, 0.28]} /><meshStandardMaterial color="#facc15" /></mesh>;
      })}
    </>
  );
}

function MachineBody({ type, color }) {
  if (type === 'robot') return <><mesh position={[0,0.1,0]}><cylinderGeometry args={[0.28,0.42,0.7,24]} /><meshStandardMaterial color={color} metalness={0.25} roughness={0.35} /></mesh><mesh position={[0,0.65,0]}><sphereGeometry args={[0.32,24,24]} /><meshStandardMaterial color={color} metalness={0.2} roughness={0.35} /></mesh><mesh position={[0.45,0.45,0]} rotation={[0,0,0.8]}><boxGeometry args={[0.65,0.16,0.16]} /><meshStandardMaterial color="#475569" /></mesh></>;
  if (type === 'cutter') return <><mesh><cylinderGeometry args={[0.5,0.5,0.9,32]} /><meshStandardMaterial color={color} metalness={0.2} roughness={0.35} /></mesh><mesh position={[0,0.55,0]}><torusGeometry args={[0.38,0.04,12,32]} /><meshStandardMaterial color="#e5e7eb" /></mesh></>;
  if (type === 'packer') return <><mesh><boxGeometry args={[1.1,0.85,1.1]} /><meshStandardMaterial color={color} metalness={0.15} roughness={0.45} /></mesh><mesh position={[0,0.55,0]}><boxGeometry args={[0.75,0.12,0.75]} /><meshStandardMaterial color="#e5e7eb" /></mesh></>;
  return <><mesh><boxGeometry args={[1.1,0.8,1.1]} /><meshStandardMaterial color={color} metalness={0.25} roughness={0.35} /></mesh><mesh position={[0,0.52,0]}><boxGeometry args={[0.7,0.18,0.7]} /><meshStandardMaterial color="#cbd5e1" /></mesh></>;
}

function Machine({ m, selected, onSelect, onMove, moveMode }) {
  const t = types.find(x => x.id === m.type) || types[0];
  return (
    <group position={[m.x, 0.5, m.z]} onClick={(e) => { e.stopPropagation(); onSelect(m.id); }}>
      <MachineBody type={m.type} color={t.color} />
...
export default function App() {
  ...
  return (
    <div style={{height:'100vh',fontFamily:'Arial,sans-serif',background:'#0f172a',overflow:'hidden'}}>
      <main style={{height:'100vh'}}>
        <Canvas camera={{ position: [8, 8, 8], fov: 45 }}>
          <ambientLight intensity={0.75} /><directionalLight position={[5,10,5]} intensity={1.1} />
          <gridHelper args={[24,24]} />
          <mesh rotation={[-Math.PI/2,0,0]} onClick={(e)=>addOrMove(snap(e.point.x),snap(e.point.z))}><planeGeometry args={[24,24]} /><meshStandardMaterial color="#f8fafc" /></mesh>
          {belts.map(b => <Conveyor key={b.id} from={byId[b.from]} to={byId[b.to]} running={running} speed={speed} time={time} />)}
          {machines.map(m => <Machine key={m.id} m={m} selected={sel===m.id} onSelect={select} moveMode={moveMode} />)}
          <OrbitControls />
        </Canvas>
      </main>
      ...
      <aside style={{position:'fixed',left:12,right:12,bottom:12,maxHeight:'44vh',overflowY:'auto',background:'rgba(255,255,255,.96)',borderRadius:18,padding:12,boxShadow:'0 20px 40px rgba(0,0,0,.25)'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginTop:10}}>
          <button onClick={()=>setRunning(!running)} style={btn}>{running?'Pause':'Start'}</button>
          <button disabled={!selected} onClick={()=>setMoveMode(true)} style={btn}>Move</button>
          <button disabled={!selected} onClick={()=>setConnectFrom(selected.id)} style={btn}>Connect</button>
        </div>
